...
public class MainActivity extends AppCompatActivity {
            ...
            for (int i = 0; i < figUnder.getPointsSize(); i++) {
                ...
                if (cutEdges.get(i).size() > 3) {
                    for (int u = 0; u < cutEdges.get(i).size() - 1 ; u++) {
                        for(int l = u + 1; l < cutEdges.get(i).size(); l++) {
                            if ((cutEdges.get(i).get(u).getX() == cutEdges.get(i).get(l).getX()) &&
                                    (cutEdges.get(i).get(u).getY() == cutEdges.get(i).get(l).getY())) {
                                cutEdges.get(i).remove(l);
                                l--;
                                u--;
                                break;
                            }
                ...
                cutEdges.set(i, edgeSort(cutEdges.get(i)));
            }
        } else {
            for (int i = 0; i < figUnder.getPointsSize(); i++) {
                ArrayList<MyPoint> a = new ArrayList<>();
    ...
    ArrayList<MyPoint> edgeSort(ArrayList<MyPoint> oldCutEdge){
        ArrayList<Float> ts = new ArrayList<>();
        ArrayList<MyPoint> newCutEdge = new ArrayList<>();
        ts.add(0f);
        for (int i = 1; i < oldCutEdge.size() - 1; i++){
            ts.add((oldCutEdge.get(i).getX() - oldCutEdge.get(0).getX())/
                    (float)(oldCutEdge.get(oldCutEdge.size() - 1).getX() - oldCutEdge.get(0).getX()));
        }
        ts.add(1f);
        for (int i = 0; i < oldCutEdge.size(); i++){
            int tmin = ts.indexOf(min(ts));
            newCutEdge.add(oldCutEdge.get(tmin));
            ts.set(tmin, 2f);
        }
        return newCutEdge;
    }
    ...
    public void zoom(Figura figura, Boolean z) {
        ...
        int[] cent = new int[2];
        cent[0] = figura.centerPoint.getX();
        cent[1] = figura.centerPoint.getY();
            ...
            for (int i = 0; i < arrFigura.size(); i++) {
                if (arrFigura.get(i).getForeground()) {
                    ...
                    for (int t = 0; t < figura.getPointsSize(); t++) {
                        figura.changePoint(figura.getPoint(t).getX() - 20, figura.getPoint(t).getY(), t);
                    }
                    arrFigura.get(i).getCenterPoint();
                }
                if (!arrFigura.get(i).getConvex()) {
                    arrFigura.get(i).triangulation();
                }
        ...
        if(!arrFigura.isEmpty()) {
            for (int i = 0; i < arrFigura.size(); i++) {
                if (arrFigura.get(i).getForeground()) {
                    Figura figura = arrFigura.get(i);
                    for (int t = 0; t < figura.getPointsSize(); t++) {
                        figura.changePoint(figura.getPoint(t).getX(), figura.getPoint(t).getY() - 20, t);
                    }
                    arrFigura.get(i).getCenterPoint();
                }
                if (!arrFigura.get(i).getConvex()) {
            ...
            for (int i = 0; i < arrFigura.size(); i++) {
                if (arrFigura.get(i).getForeground()) {
                    ...
                    for (int f = 0; f < figura.getPointsSize(); f++) {
                        double x = cent[0] + (figura.getPoint(f).getX() - cent[0]) * Math.cos(Math.PI / 12) - (figura.getPoint(f).getY() - cent[1]) * Math.sin(Math.PI / 12);
                        double y = cent[1] + (figura.getPoint(f).getX() - cent[0]) * Math.sin(Math.PI / 12) + (figura.getPoint(f).getY() - cent[1]) * Math.cos(Math.PI / 12);
                        figura.changePoint((int) x, (int) y - 1, f);
                    }
                }
                if (!arrFigura.get(i).getConvex()) {
                    arrFigura.get(i).triangulation();
